...
def setup_command(
  ...
  typer.secho(
      "AgentSpace IAM Permissions Setup", fg=typer.colors.CYAN, bold=True
  )
  typer.secho("=" * 50, fg=typer.colors.CYAN)
  typer.echo("")
  ...
  if dry_run:
    typer.secho(
        "DRY RUN MODE - No changes will be made", fg=typer.colors.YELLOW
    )
    typer.echo("")
  ...
  # Setup permissions
  results = manager.setup_agentspace_permissions(
  ...
  if results["existing"]:
    typer.secho(
        f"Already configured: {len(results['existing'])}",
        fg=typer.colors.YELLOW,
    )
...
def verify_command(
  ...
  if all_configured:
    ...
    typer.echo("")
    typer.echo(
        "Run 'python manage.py iam setup' to configure missing permissions."
    )
    raise typer.Exit(code=1)
...
def list_roles_command(
  ...
  if roles:
    typer.secho(
        f"Granted Roles ({len(roles)}):", fg=typer.colors.GREEN, bold=True
    )
    for role in roles:
      typer.echo(f"  - {role}")
  else:
