...
def deploy(
    agent_module: str = typer.Option(
        "agent", help="Module containing create_agent()"
    ),
    description: str = typer.Option(
        "OneMCP SecOps Agent with OAuth Passthrough", help="Agent description"
    ),
    debug: bool = typer.Option(False, "--debug", help="Enable debug logging"),
    run_test: bool = typer.Option(
        True, "--test/--no-test", help="Run a test query after deployment"
    ),
):
  ...
  try:
    ...
    try:
      ...
      client_options = {
          "api_endpoint": f"{os.environ.get('GCP_LOCATION', 'us-central1')}-aiplatform.googleapis.com"
      }
      ...
      existing_engine = client.get_reasoning_engine(
      ...
      reasoning_engine_resource = aiplatform_v1beta1.ReasoningEngine(
          name=remote_app.resource_name, spec=new_spec
      )

      typer.echo(
          "Tagging engine as 'google-adk' and applying telemetry vars..."
      )
      client.update_reasoning_engine(
          reasoning_engine=reasoning_engine_resource, update_mask=update_mask
      ).result()
      typer.secho("Tagging successful!", fg=typer.colors.GREEN)
    except Exception as label_err:
      typer.secho(
          f"Warning: Failed to add ADK labels: {label_err}",
    ...
    if run_test:
      typer.echo("\nRunning smoke test...")
      try:
        response = query_reasoning_engine(remote_app, "Hello")
        typer.echo(f"Test response: {response}")
      except Exception as test_err:
        typer.secho(
            f"Warning: Smoke test failed: {test_err}", fg=typer.colors.YELLOW
        )
...
def update(
  ...
  if not resource_name:
    typer.secho(
        "Error: AGENT_ENGINE_RESOURCE_NAME not set in .env", fg=typer.colors.RED
    )
    raise typer.Exit(1)
  ...
  try:
    ...
    try:
      ...
      typer.secho(
          f"Warning: Failed to update ADK labels: {label_err}",
          fg=typer.colors.YELLOW,
      )
...
def tag_as_adk(
    resource_name: str = typer.Option(
        None, help="Specific resource name to tag (defaults to .env value)"
    ),
):
  ...
  if not name:
    typer.secho(
        "Error: No resource name provided and AGENT_ENGINE_RESOURCE_NAME not set in .env",
        fg=typer.colors.RED,
    )
    raise typer.Exit(1)
  ...
  try:
    ...
    reasoning_engine_resource = aiplatform_v1beta1.ReasoningEngine(
        name=name, spec=new_spec
    )

    typer.echo(
        f"Tagging engine {name} as 'google-adk' and applying telemetry vars..."
    )
    client.update_reasoning_engine(
        reasoning_engine=reasoning_engine_resource, update_mask=update_mask
    ).result()
    typer.secho(
        "Tagging successful! Cloud Console metrics should now be available.",
...
def delete(
  ...
  if resource:
    try:
      ...
      typer.secho(
          f"Error: Could not find engine with resource name {resource}: {e}",
          fg=typer.colors.RED,
      )
      raise typer.Exit(1)
    ...
    try:
      ...
      if index < 1 or index > len(engines):
        typer.secho(
            f"Error: Invalid index {index}. Use 'list' to see valid indices.",
            fg=typer.colors.RED,
        )
        raise typer.Exit(1)
    ...
    typer.secho(
        "Error: Must provide either --index or --resource", fg=typer.colors.RED
    )
    raise typer.Exit(1)

  if not force:
    if not typer.confirm(
        f"Are you sure you want to delete engine {target_engine.resource_name}?"
    ):
...
def query_reasoning_engine(engine_or_name, prompt: str) -> str:
  """Query an ADK ReasoningEngine instance via vertexai.agent_engines."""
  import google.auth
  ...
  resource_name = (
      ...
      else getattr(engine_or_name, "resource_name", str(engine_or_name))
  )
  ...
  if gemini_auth_id:
    try:
      ...
      if creds.token:
        state[gemini_auth_id] = creds.token
    except Exception:
      pass

  session = ae.create_session(user_id="test_user", state=state)
  chunks = list(
      ae.stream_query(
          user_id="test_user", session_id=session["id"], message=prompt
      )
  )
  texts = []
  for chunk in chunks:
  ...
  return "".join(texts) if texts else str(chunks)
...
def test(
  ...
  try:
    response = query_reasoning_engine(resource_name, input)
    typer.echo(f"\nResponse: {response}")
...
def load_agent(module_name: str):
  ...
  try:
    ...
    if hasattr(module, "create_agent"):
      ...
      typer.secho(
          f"Error: Module '{module_name}' has no create_agent() function",
          fg=typer.colors.RED,
      )
      raise typer.Exit(1)
  except Exception as e:
    typer.secho(
        f"Error loading agent module '{module_name}': {e}", fg=typer.colors.RED
    )
    import traceback
